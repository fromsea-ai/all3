...
vector<Pair> aStarSearch(Pair start, Pair goal){
    ...
    while(!open.empty()){
        ...
        for(auto d : dir){
            ...
            if(!isRange(next) || !isUnblocked(next) || closed.count(next)) continue;
            double newG = gCost[current] + 1;

            if(!gCost.count(next) || newG < gCost[next]){
                gCost[next] = newG;
                parent[next] = current;
                double f = newG + h(next, goal);
    ...
    do{
        ...
        switch (choice){
            ...
            case 2: {
                ...
                if(!isRange(src) || !isUnblocked(src) || !isRange(dest) || !isUnblocked(dest)){
                    cout << "Invalid Coordinates" << endl;
                    hasSD = false;
                }
                else {
